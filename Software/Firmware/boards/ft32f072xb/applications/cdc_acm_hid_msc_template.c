...
#define USBD_VID           0x0584
#define USBD_PID           0x0304
...
void cdc_acm_hid_msc_descriptor_init(uint8_t busid, uintptr_t reg_base)
{
    ...
    usbd_add_interface(busid, usbd_hid_init_intf(busid, &intf2, hid_mouse_report_desc, HID_MOUSE_REPORT_DESC_SIZE));
...
{
    if (dtr_enable) {
        memset(&write_buffer[0], 'a', 64);
        ep_tx_busy_flag = true;
        usbd_ep_start_write(busid, CDC_IN_EP, write_buffer, 64);
