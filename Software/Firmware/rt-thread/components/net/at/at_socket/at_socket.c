...
__exit:
    ...
    if (sock != RT_NULL)
    {
        if (recv_len > 0)
        {
            ...
            {
                /* TODO:
                    检查socket recvpkt_list为空后重置recv_notice信号量
                */
                rt_sem_control(sock->recv_notice, RT_IPC_CMD_RESET, 0);
...
struct hostent *at_gethostbyname(const char *name)
{
    ...
    {
        strncpy(ipstr, name, sizeof(ipstr));
    }
    ...
    *res = RT_NULL;
    ...
    if (nodename != RT_NULL)
    {
        ...
        else
        {
            ...
            {
                strncpy(ip_str, nodename, sizeof(ip_str));
            }
