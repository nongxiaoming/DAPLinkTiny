...
#endif
    OTG->GINTSTS = ISTR_RESUME;
  }

/* speed enumeration completed                                                */
  if (istr & ISTR_ENUMDONE) {
    OTG->DIEPCTL0 &= ~3;
...
/* End of periodic frame                                                      */
  if (istr & ISTR_EOPF) {
    for (num = 1; num <= USBD_EP_NUM; num++) {
      ...
      if (OTG->GINTSTS & ISTR_INCOMISOOUTM) {
    ...
    OTG->GINTSTS = ISTR_EOPF | ISTR_INCOMISOOUTM;
  }

/* incomplete isochronous IN transfer                                         */
  if (istr & ISTR_INCOMISOIN) {
    OTG->GINTSTS = ISTR_INCOMISOIN;
