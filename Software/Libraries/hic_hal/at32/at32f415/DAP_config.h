...
__STATIC_INLINE void pin_out_init(GPIO_Type* GPIOx, uint8_t pin_bit)
{
    if(pin_bit >= 8)
    {
        GPIOx->CTRLH &= ~(0x0000000F << ((pin_bit-8) << 2));
        GPIOx->CTRLH |= ( ((uint32_t)(0x00|0x03) & 0x0F) << ((pin_bit-8) << 2) );
    }
    else
    {
        GPIOx->CTRLL &= ~(0x0000000F << ((pin_bit) << 2));
        GPIOx->CTRLL |= ( ((uint32_t)(0x00|0x03) & 0x0F) << ((pin_bit) << 2) );
    }
}

__STATIC_INLINE void pin_out_od_init(GPIO_Type* GPIOx, uint8_t pin_bit)
{
    if(pin_bit >= 8)
    {
        GPIOx->CTRLH &= ~(0x0000000F << ((pin_bit-8) << 2));
        GPIOx->CTRLH |= ( ((uint32_t)(0x04|0x03) & 0x0F) << ((pin_bit-8) << 2) );
    }
    else
    {
        GPIOx->CTRLL &= ~(0x0000000F << ((pin_bit) << 2));
        GPIOx->CTRLL |= ( ((uint32_t)(0x04|0x03) & 0x0F) << ((pin_bit) << 2) );
    }
}

__STATIC_INLINE void pin_in_init(GPIO_Type* GPIOx, uint8_t pin_bit, uint8_t mode)
{
    uint8_t config;
    if(mode == 1)
        config = 0x08; //Up
    ...
    {
        GPIOx->CTRLH &= ~(0x0000000F << ((pin_bit-8) << 2));
        GPIOx->CTRLH |= ( ((uint32_t)(config) & 0x0F) << ((pin_bit-8) << 2) );
        if(mode == 1)
            GPIOx->BSRE = (((uint32_t)0x01) << pin_bit);
        else if(mode == 2)
            GPIOx->BRE = (((uint32_t)0x01) << pin_bit);
    }
    else
    {
        GPIOx->CTRLL &= ~(0x0000000F << ((pin_bit) << 2));
        GPIOx->CTRLL |= ( ((uint32_t)(config) & 0x0F) << ((pin_bit) << 2) );
        if(mode == 1)
            GPIOx->BSRE = (((uint32_t)0x01) << pin_bit);
        else if(mode == 2)
            GPIOx->BRE = (((uint32_t)0x01) << pin_bit);
...
{
    SWCLK_TCK_PIN_PORT->BRE = SWCLK_TCK_PIN;
}
...
{
    return ((SWDIO_IN_PIN_PORT->IPTDT & SWDIO_IN_PIN) ? 1 : 0);
}
...
{
    SWDIO_OUT_PIN_PORT->BRE = SWDIO_OUT_PIN;
}
...
{
    return ((nRESET_PIN_PORT->IPTDT >> nRESET_PIN_Bit) & 1);
}
...
__STATIC_INLINE void     PIN_nRESET_OUT(uint32_t bit)
{
    if (bit & 1)
        ...
        nRESET_PIN_PORT->BRE = nRESET_PIN;
}
...
{
    if (bit & 1)
        CONNECTED_LED_PORT->BRE = CONNECTED_LED_PIN; // LED on
    else
        CONNECTED_LED_PORT->BSRE = CONNECTED_LED_PIN;// LED off
}
...
__STATIC_INLINE void DAP_SETUP(void)
{
    ...
    CONNECTED_LED_PORT->BSRE = CONNECTED_LED_PIN;
}
